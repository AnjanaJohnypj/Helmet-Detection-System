...
#print(os.sys.path)
classesFile = r"C:\Users\007al\OneDrive\Desktop\object-detection\yolo_helmet_detection\obj.names";
classes = None
with open(classesFile, 'rt') as f:
    classes = f.read().rstrip('\n').split('\n')

# Give the configuration and weight files for the model and load the network using them.
modelConfiguration = r"C:\Users\007al\OneDrive\Desktop\object-detection\yolo_helmet_detection\yolov3-obj.cfg";
modelWeights = r"C:\Users\007al\OneDrive\Desktop\object-detection\yolo_helmet_detection\yolov3-obj_2400.weights";
...
def drawPred(classId, conf, left, top, right, bottom):
    ...
    global frame_count, label_global
# Draw a bounding box.
    ...
    if label_name == 'Helmet':
       ...
       # cv.rectangle(frame, (left, top - round(1.5*labelSize[1])), (left + round(1.5*labelSize[0]), top + baseLine), (255, 255, 255), cv.FILLED)
        #scv.putText(frame, label, (left, top), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,0), 1)
        cv.rectangle(frame, (left, top ), (left , top ), (255, 255, 255), cv.FILLED)
        frame_count+=1
    label_global = label_name
...
#for fn in glob('../../images/*.jpg'):
    #frame = cv.imread(fn)
def detect_helemt(lframe):

    frame_count =0
    global frame, img_global, complete, label_global
    label_global = None
    frame = lframe
    complete = False
